...
const CompanyListItemCard = ({ company }: { company: Company }) => {
  ...
  return (
    <Card
      className="p-4 flex items-center gap-2 text-muted-foreground  hover:shadow-md 
      cursor-pointer"
      onClick={() => router.push(`/companies/${company.id}`)}
    >
      <h2 className="tracking-wide whitespace-nowrap font-semibold text-black truncate">
        {company.name}
      </h2>
    </Card>
  );
