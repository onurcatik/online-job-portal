...
export const ImageUpload = ({
  ...
  /**
   * Dosya seçildiğinde Cloudinary’ye yükleme işlemi başlatılır.
   * Yükleme sırasında progress takibi yapılır.
   */
  const onUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!e.target.files) return;
    const file: File = e.target.files[0];
    ...
    // Dosya adındaki boşluk ve özel karakterleri temizliyoruz
    const sanitizedFileName = file.name
      .replace(/\s+/g, "_")
      .replace(/[^\w.-]/g, "");

    // Cloudinary yüklemesi için FormData oluşturuluyor
    const formData = new FormData();
    formData.append("file", file);
    formData.append(
      "public_id",
